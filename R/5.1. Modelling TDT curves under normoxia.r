...
# Modified by Wilco Verberk (20210225)
# Modified by Félix P Leiva (20210226)
# -------------------------------------------------------------------------------------
...
10^(min(all.lines.21$surv.time))#lower range in minutes
10^(max(all.lines.21$surv.time))#upper range in minutes
10^(median(all.lines.21$surv.time))#median in minutes
...
count(d21.median$stock)# no data for females of the stock #28180 at 37 and 38°C
...
# Stress temp + Stress temp * interocular distance 5 -150.45      17.43   0.00   1.00 80.42
...
# ST + ST * sex + ST * interocular distance 7 -163.55      10.84   0.00   1.00 89.15

# model that includes sex and relative fresh mass works best
# ------------------------------------------------------------------------------
...
#What is the difference between these two models interm of the AIC?
AIC(fmod1)-AIC(mod1)#0.1440353

#The difference is low if we look at the r squared of both models
summary(fmod1)$r.squared-summary(m4.3)$r.squared
#Given say this and based the most parsimonious model is the "m4.3"
...
fit.list.table.2 <- list(m0,m1.1,m1.2,m1.3,m1.4,m1.5,
                       m2.1,m2.2,m2.3,m2.4,
                       m3.1,m3.2,m3.3,
                       m4.1,m4.2,m4.3
                       #full
...
#export table
write.csv(fit.table.2,"Table 2. Model comparison of survival time at 21 kPa.csv",row.names = FALSE)
...
{
pdf("Figure 4 Thermal death time curves at 21 kPa.pdf",width = 10,height = 5,useDingbats = FALSE)
png("Figure 4 Thermal death time curves at 21 kPa.png",width = 10,height = 5,units = "in",res = 300)
